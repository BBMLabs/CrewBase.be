...
export function Toaster() {
  ...
  return (
    <div
      aria-live="polite"
      className="pointer-events-none fixed inset-x-0 bottom-4 z-[1300] flex flex-col items-center gap-2 px-4"
    >
      {items.map((item) => (
        <div
          key={item.id}
        ...
        >
          ...
          {item.traceId !== undefined ? (
            <span className="ml-auto shrink-0 font-mono text-[10px] text-ink-3">{item.traceId}</span>
          ) : null}
          <button
            type="button"
